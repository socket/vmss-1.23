...
int main (int argc, const char * argv[]) {
  ...
  while (prbuff = fgets(rbuff, sizeof(rbuff), hfile)) {
    vmss_strchop(prbuff);
    if ( '\0' != *prbuff ) {
      fprintf(hfile_dst, "%24s", prbuff);
      if ( FAILED( vmss_str2number(prbuff, SRC_BASE, DST_BASE, convbuff, PRECISION) ) ) {
        sprintf(convbuff, "invalid");
      }
  ...
  fclose(hfile);
  if ( hfile_dst != 0 && hfile_dst != stdout) {
    fclose(hfile_dst);
  }
